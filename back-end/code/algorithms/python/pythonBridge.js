...
async function callPython(funcName, params = {}) {
  return new Promise((resolve, reject) => {
    ...
    console.log(`[Python] 调用 ${funcName}...`);
    ...
    child.on('close', (code) => {
      if (code !== 0) {
        ...
        console.error('[Python] stderr:', stderr.substring(0, 2000));
        reject(new Error(`Python process exited with code ${code}: ${stderr.substring(0, 500)}`));
        return;
      }
      ...
      try {
        ...
        if (startIdx === -1 || endIdx === -1) {
          ...
          reject(new Error('Python output missing result markers'));
          return;
        }
        ...
        if (!result.success) {
          ...
          reject(new Error(`Python algorithm error: ${result.error}`));
          return;
        }
