...
def calibrate_k(body_weight_kg, static_frames):
    # body_weight_kg: 用户体重, static_frames: 静止期采集到的压力帧列表
    # 返回: 压力校准系数 GLOBAL_K
    """
    校准系数k
    
    参数：
        body_weight_kg: 体重（公斤）
        static_frames: 静态帧列表，每帧是ADC值数组
...
def adc_to_force(adc_values):
    # adc_values: 原始 ADC 数值（矩阵或序列）
    # 返回: 转换后的牛顿力 (N) 矩阵或序列
    """
    将ADC值转换为力
    
    参数：
        adc_values: ADC值（单个数字或数组）
        k: 校准系数
...
def calculate_balance_features(S2, S3, S5, S6):
    ...
    """
    计算平衡特征，输入为原始ADC序列，内部转换为牛顿力(N)进行计算。
    S2, S3: 前足内外侧
    S5, S6: 足跟内外侧
    """
    F2 = adc_to_force(S2)
