...
握力判断统一采用 AWGS 标准（以单手峰值握力为准）：
- 男性峰值握力 < 28kg 提示握力偏低
- 女性峰值握力 < 18kg 提示握力偏低

引用标准时必须写完整，禁止出现"男性≥28kg"这种省略写法。
正确示例："依据 AWGS 标准，男性峰值握力≥28kg 为正常；本例左右手均高于阈值，提示握力正常"。
错误示例："AWGS 男性≥28kg 为正常"——缺少"峰值握力"四个字，读者无法理解 28kg 指的是什么。
...
{{
  ...
  "overview": "整体概括本次握力测试结果与关键发现，包含峰值握力水平、AWGS达标判断、主要风险方向（90~120字）",
  "left_hand_analysis": "只分析左手，不要夹带右手内容；如果左手缺失，就明确说明缺失。从峰值握力、发力速度、达峰耗时、峰值持续、手指分区贡献、抖动与姿态稳定性等多角度展开（90~120字）",
  "right_hand_analysis": "只分析右手，不要夹带左手内容；如果右手缺失，就明确说明缺失。从峰值握力、发力速度、达峰耗时、峰值持续、手指分区贡献、抖动与姿态稳定性等多角度展开（90~120字）",
  "bilateral_comparison": "只分析左右手差异、优势侧、对称性，不要重复单手细节；从峰值比值、总力差异、发力模式差异、手指分区差异等角度展开；如果只有单手数据，说明无法对比（90~120字）",
  "clinical_suggestion": "给出2~3条训练与干预建议，每条40~60字；如果左右手情况不同，要明确分别说明重点",
