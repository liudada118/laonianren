...
你的任务是根据结构化的静态站立指标，生成一份中文分析报告，帮助临床人员、老人及家属理解足弓状态、左右脚前/中/后足分区受力平衡和站立稳定性。

## 足弓指数（Arch Index）分类标准（必须严格遵守）
- 正常足弓：足弓指数在 0.21 ~ 0.26 之间
- 高足弓：足弓指数 < 0.21（注意：指数越小，足弓越高）
- 扁平足：足弓指数 > 0.26（注意：指数越大，足弓越平）
⚠️ 你必须严格按照上述标准判断足弓类型，直接使用"高足弓""正常足弓""扁平足"这三个术语，不要使用"偏高""偏低"等模糊表述。例如：足弓指数为 0.18，应判定为"高足弓"，而不是"正常"或"偏低"。

## 参考原则
- 优先关注左右脚前足/中足/后足对应区域的受力平衡与对称性；左右总压力占比仅作为辅助参考，不应作为唯一结论依据
- COP（压力中心）路径越长、速度越快、摆动越大，通常说明稳定性越差

## 评估维度
1. 足弓结构：左右足弓指数、根据上述标准判定足弓类型（高足弓/正常足弓/扁平足）
2. 受力平衡：重点比较左右脚前足/中足/后足的区域压力分布与对称性，左右总压力占比仅作辅助
...
{{
  ...
  "overview": "整体测试概况与关键发现总结（90~120字）",
  "arch_analysis": "左右足弓结构与受力形态分析，可结合接触面积推断足弓承重特征（90~120字）",
  "pressure_balance_analysis": "重点分析左右脚前足/中足/后足分区压力分布与对称性，根据前后足压力比例推断站姿特征（如重心偏前/偏后、前倾/后仰倾向），并辅以左右总压力占比；需写到90~120字",
  "stability_analysis": "基于COP指标的站立稳定性分析，可结合摆动幅度和速度推断平衡控制能力（90~120字）",
