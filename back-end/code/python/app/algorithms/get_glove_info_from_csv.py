...
# 三段式分段线性模型参数（12点标定）
# ==============================================
ADC_BREAKPOINT_1 = 61.2   # 第一转折点ADC值（对应50N）
ADC_BREAKPOINT_2 = 75.0   # 第二转折点/饱和ADC值（对应150N）
ADC_OFFSET = 2.87         # 起点偏移
CALIBRATION_POINTS = 12   # 标定点数
...
def adc_to_force_single_point(adc):
    """三段式单点模型：将单个传感器点的ADC转换为力(N)，除以标定点数"""
    if adc <= ADC_OFFSET:
        return 0.0
...
def process_glove_data_from_content(csv_content, hand_type, output_dir=None):
    """
    从 CSV 文本内容处理手套数据（供 API 调用）

    Args:
        csv_content: CSV 文件的文本内容
        hand_type: '左手' 或 '右手'
        output_dir: 输出目录（PDF/PNG），为 None 则使用临时目录

    Returns:
        dict: 包含分析指标和时序数据的结构化结果
    """
    import tempfile
    ...
    try:
        result = _process_glove_data_core(tmp_csv.name, output_csv, hand_type)
        return result
    finally:
        os.unlink(tmp_csv.name)


def _process_glove_data_core(input_csv, output_csv, hand_type):
    """核心处理逻辑（同时被 process_glove_data 和 process_glove_data_from_content 调用）"""
    ...
    try:
        ...
        raise ValueError(f"无法读取CSV: {e}")

    if 'sensor_data_calibrated' not in header:
        raise ValueError("未找到 sensor_data_calibrated 列")
    ...
    if peak_info:
        time_analysis.extend([
            {'label': '峰值力时间', 'value': f'{peak_info["peak_time"]:.3f} s'},
            {'label': '到达峰值耗时', 'value': f'{peak_info["peak_time"] - grip_start_time:.3f} s'},
            {'label': '峰值区间开始', 'value': f'{peak_info["start_time"]:.3f} s'},
            {'label': '峰值区间结束', 'value': f'{peak_info["end_time"]:.3f} s'},
            {'label': '峰值持续时间', 'value': f'{peak_info["duration"]:.3f} s'},
            {'label': '峰值力', 'value': f'{peak_info["peak_force"]:.2f} N'},
        ])
    time_analysis.extend([
        {'label': '检测阈值', 'value': f'{SHAKE_ANGULAR_VELOCITY_THRESHOLD}°/s'},
        {'label': '窗口大小', 'value': f'{ANGULAR_VELOCITY_WINDOW_SIZE} 帧'},
        {'label': '抖动次数', 'value': f'{shake_count} 次'},
        {'label': '平均角速度', 'value': f'{np.mean(angular_velocities):.2f}°/s'},
        {'label': '最大角速度', 'value': f'{np.max(angular_velocities):.2f}°/s'},
    ])
