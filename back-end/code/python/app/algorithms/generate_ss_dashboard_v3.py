...
def calculate_metrics_stand(frames, center_l, center_r, interval_sec):
    ...
    for i, frame in enumerate(frames):
        ...
        raw_fl = frame * mask_l
        fl = np.where(raw_fl < 150, raw_fl / 12.7, 12.0)
        ...
        raw_fr = frame * mask_r
        fr = np.where(raw_fr < 150, raw_fr / 12.7, 12.0)
...
def calculate_metrics_sit(frames, times):
    ...
    for i, frame in enumerate(frames):
        s_sum = sum_curve[i]
        # ADC转牛顿
        s_sum_newton = s_sum / 26.18
        metrics['sum'].append(s_sum_newton)
...
def generate_combined_dashboard(
    ...
    sl_objs = [
        setup_chart(ax_stand_l[0], "左脚压力（N）", t_vec_stand, l_metrics['sum'], 'cyan'),
    ...
    sr_objs = [
        setup_chart(ax_stand_r[0], "右脚压力（N）", t_vec_stand, r_metrics['sum'], 'cyan'),
    ...
    sit_objs = [
        setup_chart(ax_sit_charts[0], "坐姿压力（N）", t_vec_sit, sit_metrics['sum'], 'cyan'),
...
def generate_video_from_content(stand_csv_content, sit_csv_content, output_path, speed_factor=0.5):
    """
    从 CSV 文本内容生成动态视频（供 API 调用）
    """
    import tempfile, shutil

    tmp_dir = tempfile.mkdtemp(prefix='ss_video_')
    stand_csv = os.path.join(tmp_dir, "stand.csv")
    sit_csv = os.path.join(tmp_dir, "sit.csv")
    ...
    try:
        stand_frames, stand_times, _ = load_data_generic(stand_csv, shape=(64, 64), is_sit=False)
        sit_frames, sit_times, _ = load_data_generic(sit_csv, shape=(32, 32), is_sit=True)
...
if __name__ == "__main__":
    ...
    output_path = os.path.join(base_dir, "combined_dashboard_v3_N.mp4")
