...
### 7.1. 评估报告与历史记录功能修复 (2026-02-27)

本次更新集中修复了评估报告生成和历史记录查看中的多个关键 bug，并对相关算法进行了优化。

#### 7.1.1. 静态站立评估报告数据显示为 0

- **问题**: 静态站立评估完成后，报告页面所有指标（如足长、足宽、COP 参数等）均显示为 0.00。
- **根因**: 前端 `StandingReport.jsx` 组件期望的数据格式与后端 Python 算法实际返回的 `render_data` 格式不匹配。例如，前端尝试读取 `r.left.length`，但后端返回的是 `r.additional_data.left_length`。
- **修复**: 修改了 `StandingReport.jsx` 中的 `useMemo` 钩子，增加了对后端 `render_data` 格式的适配逻辑，使其能够正确解析和展示从 Python 算法返回的数据。

#### 7.1.2. 历史报告列表显示相同数据

- **问题**: 在历史记录页面，点击不同的评估记录，打开的报告内容都是相同的，而不是各自真实采集的数据。
- **根因**: 
    1.  **数据未保存**: 在完成评估时，调用 `completeAssessment` 函数仅保存了评估完成状态，并未将生成的报告数据（`reportData`）存入数据库。
    2.  **组件未接收数据**: `HistoryReportView.jsx` 在渲染历史报告时，没有从数据库记录中提取报告数据并作为 prop 传递给对应的报告组件（如 `StandingReport`, `GripReport`），导致组件显示默认或 mock 数据。
- **修复**:
    1.  修改了所有四个评估页面（`Grip`, `Standing`, `SitStand`, `Gait`），在调用 `completeAssessment` 时，将生成的 `reportData` 一并存入 `report` 字段。
    2.  重写了 `HistoryReportView.jsx`，使其能够从数据库记录中正确提取 `report.reportData`，并传递给相应的报告组件进行渲染。

#### 7.1.3. 起坐评估次数计算错误 (145次)

- **问题**: 用户实际完成 6 次起坐，但报告显示的“完成周期”为 145 次。
- **根因**: `sit_stand_render_data.py` 中，当主算法（基于 AMPD）因环境依赖问题（如缺少 `opencv-python`）导入失败时，会触发一个 fallback 算法。该 fallback 算法对脚垫压力信号（`foot1_force`）使用了一个过于简单的峰值检测逻辑（`>=threshold && >=左邻 && >右邻`），由于信号波动小，导致将大量噪声误判为起坐峰值。
- **修复**:
    1.  **优化 Fallback 算法**: 重写了 `_fallback_generate_sit_stand_report` 函数，采用更稳健的峰值检测策略：
        - **优先使用坐垫数据**: 改为优先检测坐垫压力（`sit_force`）的**谷值**。因为“站起”时坐垫压力会骤降至接近零，信号特征非常明显。
        - **使用 `scipy.signal.find_peaks`**: 引入 `scipy` 库进行峰值检测，并设置了合理的 `distance`（最小间距）和 `prominence`（峰值显著度）参数，有效过滤噪声。
        - **增加绝对值过滤**: 增加了对谷值绝对大小的判断，确保只有压力显著降低（低于均值的50%）的谷值才被计为“站起”。
        - **末尾周期检测**: 增加了对信号末尾的特殊处理逻辑，以正确检测最后一次站起后未再坐下的情况。
    2.  **增加日志**: 在 `try-except` 块中增加了日志记录，当主算法导入失败时会打印详细错误，便于未来排查环境问题。
